...
---@type table<string, table<string,string>>
local _ClassData = Ext.Utils.Include(nil, "builtin://Libs/HelpersGenerator/ClassFixes.lua")

---@type string
local _StartingText = Ext.Utils.Include(nil, "builtin://Libs/HelpersGenerator/StartingText.lua")
...
function Generator:EmitClass(type, moduleToClassField)
    ...
    local extraClassData = _ClassData[name] or {}
    if extraClassData then
        for field,typeName in pairs(extraClassData) do
            table.insert(sortedMembers, field)
        end
    end
    ...
    for i,fname in ipairs(sortedMembers) do
        local typeSig = ""
        if extraClassData and extraClassData[fname] then
            typeSig = extraClassData[fname]
        else
            typeSig = self:MakeTypeSignature(type, type.Members[fname])
        end
